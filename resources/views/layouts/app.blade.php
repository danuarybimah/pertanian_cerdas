    ...
    <nav class="navbar">
        <div class="navbar-inner">
            ...
            <div class="navbar-menu" id="nav-menu"
                style="@auth @if (!auth()->user()->isPetani()) display:none; @endif @endauth">
                @auth
                    <a href="{{ route('dashboard') }}"
                        class="{{ request()->routeIs('dashboard') ? 'active' : '' }}">Beranda</a>
                    <a href="{{ route('konsultasi.index') }}"
                        class="{{ request()->routeIs('konsultasi*') ? 'active' : '' }}">Konsultasi</a>
                @endauth
                <a href="{{ route('harga-pasar.index') }}"
                    class="{{ request()->routeIs('harga-pasar*') ? 'active' : '' }}">Harga Pasar</a>
                <a href="{{ route('artikel.index') }}"
                    class="{{ request()->routeIs('artikel*') ? 'active' : '' }}">Artikel</a>
                <a href="{{ route('kalender-tanam.index') }}"
                    class="{{ request()->routeIs('kalender*') ? 'active' : '' }}">Kalender Tanam</a>
            </div>
            <div class="navbar-actions">
                    ...
                    <div class="profile-dropdown">
                        ...
                        <div class="profile-dropdown-menu" id="profileDropdownMenu">
                            @if (auth()->user()->isPetani())
                                ...
                                <a href="{{ route('konsultasi.index') }}"><i data-lucide="message-square"></i>
                                    Konsultasi</a>
                                <a href="{{ route('harga-pasar.index') }}"><i data-lucide="banknote"></i> Harga Pasar</a>
                                <a href="{{ route('kalender-tanam.index') }}"><i data-lucide="calendar"></i> Kalender
                                    Tanam</a>
                ...
                @else
                    <a href="{{ route('login') }}" class="btn btn-outline btn-sm"
                        style="color:white;border-color:rgba(255,255,255,.4);">Masuk</a>
            ...
            @if (!auth()->user()->isPetani())
                <aside class="sidebar">
                    <nav class="sidebar-nav">
                        ...
                        <a href="{{ route('harga-pasar.index') }}"
                            class="{{ request()->routeIs('harga-pasar*') ? 'active' : '' }}">
                            <span class="nav-icon">💰</span> Harga Pasar
                        </a>
                        <a href="{{ route('artikel.index') }}"
                            class="{{ request()->routeIs('artikel.index') ? 'active' : '' }}">
                        ...
                        <a href="{{ route('konsultasi.index') }}"
                            class="{{ request()->routeIs('konsultasi*') ? 'active' : '' }}">
                        ...
                        @if (auth()->user()->isDinas())
                            ...
                            <a href="{{ route('dinas.users.index') }}"
                                class="{{ request()->routeIs('dinas.users*') ? 'active' : '' }}">
        ...
        <div class="footer-inner">
            <div class="footer-grid">
                <div>
                    ...
                    <p class="footer-desc">Sistem Informasi Pertanian Cerdas — portal berbasis data cuaca & pasar untuk
                        mendukung produktivitas petani lokal Indonesia.</p>
                </div>
                <div>
            ...
            <div class="footer-bottom">© {{ date('Y') }} SIPertani — Sistem Informasi Pertanian Cerdas.
                Dikembangkan untuk mendukung petani lokal Indonesia.</div>
        </div>
    </footer>
